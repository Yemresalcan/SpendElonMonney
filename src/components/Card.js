...
function Card(product) {
  ...
  return (
    <Box p={4} bg="white" boxShadow="xl">
      ...
      <Center bg="none">
        <Text bg="none" color="orange">
          {`$${new Intl.NumberFormat('en-US').format(product.item.productPrice)}`}
        </Text>
      </Center>
      <Center mt={5} bg="none">
        <Button colorScheme="orange" mr={3} w="30%" onClick={() => setCount(count - 1)} isDisabled={isSellable}>
          Sell
        </Button>
        <Input type="number" textAlign="center" w="30%" value={count} onChange={(e) => handleInputChange(parseInt(e.target.value))} />
        <Button colorScheme="red" ml={3} w="30%" onClick={() => setCount(count + 1)} isDisabled={isBuyable}>
          Buy
        </Button>
      </Center>
    </Box>
